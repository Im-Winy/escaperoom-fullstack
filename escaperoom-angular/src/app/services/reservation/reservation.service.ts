...
export class ReservationService {
  ...
  public getAvailableTimeSlotsForEvent(eventId: number, selectedDate: string): Observable<TimeSlot[]> {
    const url = `${this.host}/auth/evenement/${eventId}/timeslots?selectedDate=${selectedDate}`;  // L'URL dépend de l'API, il faut l'ajuster
  ...
  public generateSlotsForDay(date: Date): Observable<TimeSlot[]> {
    ...
    return this.http.post<TimeSlot[]>(`${this.host}/auth/generer-creneaux-journee`, null, { params });
  }

  public getReservationsByUserId(userId: number): Observable<Reservation[]> {
    return this.http.get<Reservation[]>(`${this.host}/auth/historique-reservations/user/${userId}`);
  }
  ...
  public reserve(timeSlotId: number, idEvenement: number): Observable<Reservation> {
    ...
    const url = `${this.host}/auth/reservation/${user.idUser}/${idEvenement}`;
